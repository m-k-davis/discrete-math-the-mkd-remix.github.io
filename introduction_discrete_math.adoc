...
As an example, consider a wall, a structure built from individual stones and bricks, part of which is shown in the image to the right. The 
individual 
...
It does NOT makes sense to ask about "the next real number after $\pi$ on the number line" because if we think _c_ is "the next real number after $\pi$ on the number line" then we can compute the number $c_{1} = \frac{c+\pi}{2},$ which is the midpoint of the interval with endpoints $\pi,$ so $c_{1}$ is closer to $\pi$ than _c_ is, which means $c_{1}$ is a _better candidate_ for "the next real number after $\pi$ on the number line" than _c_ is .... The concept "the next real number after $\pi$ on the number line" does not make sense for this structure as such a number cannot exist! We can just keep computing numbers that get closer and closer to $\pi$ over and over again. Likewise, "the real numbers that are closest to $\pi$" do not exist. + 
...
the set of uppercase English letters and the set of lowercase English letters is represented in the image. 
...
Notice that you could choose any lowercase letter then follow the arrow up to its corresponding uppercase letter. This shows that the one-to-one correspondence 
...
For this one-to-one correspondence, it makes sense to use arrows that point in only one direction.  
...
//|===
link:./introduction_discrete_math.html#informal_set_definition[Sets]

link:./introduction_discrete_math.html#informal_one_to_one_correspondence_definition[One-to-one correspondence]
...
link:./introduction_discrete_math.html#informal_counting_definition[Counting]
...
link:./introduction_discrete_math.html#informal_function_definition[Functions]

link:./introduction_discrete_math.html#informal_sequence_definition[Sequences]
...
link:./introduction_discrete_math.html#informal_recursion_definition[Recursion]
 link:./introduction_discrete_math.html#informal_recurrence_relation_definition[Recurrence Relations]
