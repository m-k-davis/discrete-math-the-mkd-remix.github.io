...
_inserted brief note on big Omega ($\Omega$) notation._ + 
_inserted "Check Your Understanding" for floor and ceiling functions._
...
Explain why each of the functions $\lfloor x \rfloor$ and $\lceil x \rceil$ is $\Theta(x).$ 

//(Recall that the floor and ceiling functions are described link:./introduction_discrete_math.html#informal_floor_and_ceiling_definition[here.])  
...
****
////
[small]#NOTE: There is also big Omega ($\Omega$) notation that uses only the first part of the inequality from the definition of big Theta. Saying that the  function $f$ is big Omega of $g,$ also written as $f = \Omega(g),$ means that $f$ grows at least at the rate of $g,$ that is, $g$ is a lower bound for $f$ for sufficiently large values of the input variable. For example, $0.0001 x^2 = \Omega(x)$ since a quadratic function grows at least at the rate of a linear function. Big Omega is not discussed in further detail in this textbook.#
////
[small]#NOTE: There is also big Omega ($\Omega$) notation that uses only the first part of the inequality used to define big Theta. You would say “_f_ is of order at least _g_” or “$f$ is big Omega of $g$” and write $f = \Omega(g).$ For example, $0.0001 x^2 = \Omega(x)$ since a quadratic function grows at least at the rate of a linear function; more precisely, $ x \leq 0.0001 x^2$ for all $x > 10000.$ Big Omega will not be discussed in further detail in this textbook.# 
...
[small]#However, big _O_ notation is commonly used to indicate the “tighest upper bound” possible as discussed link:https://en.wikipedia.org/wiki/Big_O_notation#Quality_of_approximations_using_different_notation[here.]# 
// MKD Apr 12 2026 this next statement is Flase! Fix it!!! 
//For example, you can expect the statement “$f$ is big-_O_ of $x$” to mean that the graph of the function $f$ has a straight line of nonzero slope as an asymptote.# 

// MKD Mar 27 2026 
// This is NOT the same as $f$ is big-Theta of $x$): 
//	Example 1. $x \sin^{2}(x)$ is $O(x)$ but not $\Omega(x)$ 
//	Example 2. $left\lceil x \right\rceil - \left\lfloor x \right\rfloor$ is $O(x)$ but not $\Omega(x)$ 
// in each example, the function has infinitely-many zeroes, so has no lower bound other than the constant function 0.


// MKD Apr 12 2026 note, but may not belong here 
// x*sin^{2}(x) is O(x) but not Big-Theta of x (it's not Big-Omega of x 
// because if has zeroes at each multiple of \pi)
