...
[small]#However, big _O_ notation is commonly used to indicate the “tighest upper bound” possible as discussed link:https://en.wikipedia.org/wiki/Big_O_notation#Quality_of_approximations_using_different_notation[here.] For example, you can expect the statement “$f$ is big-_O_ of $x$” to mean that the graph of the function $f$ has a straight line of nonzero slope as an asymptote.# 
