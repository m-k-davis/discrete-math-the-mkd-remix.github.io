...
_This chapter was last updated on February 2, 2026._ + 
...
DS2013/Basics of Counting	Counting arguments: Arithmetic and geometric progressions
DS2013/Basics of Counting	The pigeonhole principle
DS2008/BasicsOfCounting	Counting arguments
DS2008/BasicsOfCounting	Sum and product rule
DS2008/BasicsOfCounting	Inclusion-exclusion principle
DS2008/BasicsOfCounting	The pigeonhole principle
DS2001/Basics of counting	Counting arguments: Sum and product rule
DS2001/Basics of counting	Counting arguments: Inclusion-exclusion principle
DS2001/Basics of counting	The pigeonhole principle
MSF-Discrete_2023	4. Permutations, combinations, counting, pigeonhole principle
...
//This chapter introduces techniques you can use to count the number of elements of any set quickly and efficiently no matter the size of the set.

Key terms and concepts covered in this chapter:

* Counting arguments
** Sum Rule 
// (also called the Addition Principle or the Rule Of Sum)
** Product Rule
// (also called the Multiplication Principle or the Rule Of Product)
...
// MKD see https://en.wikipedia.org/wiki/Addition_principle
// MKD Epp names "Difference Rule" as |A-B| = |A|-|B| 

////
// MKD move this to the counting_binomial.adoc file
* Permutations and combinations
** Basic definitions
** Pascal’s identity
...
// MKD Jan 202 26 changed we to you
Each of the four arithmetic operations corresponds to a rule you can use to count quickly. 
...
//image::imagesMKD/set-of-playing-card.jpg[set-of-playing-card,1920,1271,float="right",align="center"]
//image::imagesMKD/set-of-playing-card_CROPPED.png[set-of-playing-card_CROPPED,1849,870,float="right",align="center"]
image::imagesMKD/set-of-playing-card_CROPPED.png[set-of-playing-card_CROPPED,277,131,float="right",align="center"]
.You Try
A card 
//is 
will be 
drawn from a link:https://en.wikipedia.org/wiki/Standard_52-card_deck[standard 52-card deck.] + 
How many ways can the card be an even number or a king? + 
[small]#Image credit: This is a cropped version of link:https://www.publicdomainpictures.net/en/view-image.php?image=65772&picture=set-of-playing-card["Set Of Playing Card"]. George Hodan has released this “Set Of Playing Card” image under Public Domain license  link:http://creativecommons.org/publicdomain/zero/1.0/[(CC0 Public Domain)].#

****

// MKD notes Jan 22 2025
//
// This first deck uses T instead of Ace, and B (Bower) instead of Jack
//https://www.publicdomainpictures.net/en/view-image.php?image=440680&picture=playing-cards
//License: CC0 Public Domain
//Sam Kal has released this “Playing Cards” image under Public Domain license. It means that you can use and modify it for your personal and commercial projects. If you intend to use an image you find here for commercial use, please be aware that some photos do require a model or a property release. Pictures featuring products should be used with care.
//
// This deck is the "standard American" or "International" deck
//https://www.publicdomainpictures.net/en/view-image.php?image=65772&picture=set-of-playing-card
//License: CC0 Public Domain
//George Hodan has released this “Set Of Playing Card” image under Public Domain license. It means that you can use and modify it for your personal and commercial projects. If you intend to use an image you find here for commercial use, please be aware that some photos do require a model or a property release. Pictures featuring products should be used with care.
...
The *subtraction rule* describes the number of possible choices of one element from a union of two sets that _do_ share one or more common elements. 
...
[small]#Notice that Dr. Roberts uses a Venn diagram to represent the sets in this video. Venn diagrams are covered in the link:./set_theory.html[Set Theory] chapter of the Remix.#
...
.Solution
Choosing a port means you first choose a computer and then a port on that computer. Since there are 27 computers and 15 ways to choose a port on a computer, 
//there are $27 \times 15 = 405$ ways to choose a port.
there are $(27)(15) = 405$ ways to choose a port.
...
//	video shows license plate with 7 letters but example is 4 letters and 3 digits
...
// MKD Example reworded Feb 02 2025
****
.Example {counter:countexample}
...
Secondly, a bitstring of length four that ends with 00 will be of the form $*~*~0~0$, so there are $(2)(2)(1)(1) = 2^2 = 4$ bitstrings of this form. + 
 + 
 Thirdly,
a bitstring of length four that starts with 1 and ends with 00 will be of the form $1~*~0~0$, so there are $(1)(2)(1)(1) = 2$ bitstrings of this form.  + 
 + 
...
//$2^3 + 2^2 - 2 = 8+4-2=10$ possibilities.
...
// MKD added  (that is, a Jack, Queen, or King)


=== The Division Rule


This rule is used when there are $n$ ways to complete a procedure, but 
//for 
each of those ways is equivalent to $d$ ways (including the way itself.) That is, every possible outcome of the procedure can be done in $d$ different ways

.The Division Rule
****
Suppose that we have a procedure that can be completed in _n_ possible ways, but that for each way of completing the procedure there are _d_ possible ways with the same outcome. Then there are $\frac{n}{d}$ ways to complete the procedure.
****
...
image::imagesMKD/DivisionRuleCircularTableImage.png[DivisionRuleCircularTableImage,152,154,float="right",align="center"]
.Example {counter:countexample} 


//image::imagesMKD/DivisionRuleCircularTableImage.png[DivisionRuleCircularTableImage,759,772,float="right",align="center"]
...
.Solution 
First use the product rule to find that there are (4)(3)(2)(1) = 24 possible ways for the 4 students to sit (For example, there are 4 choices for the “North,” then 3 choices remaining for the “East,” then 2 choices remaining for the “South,” and 1 choice remaining for the “West.”) Next, notice that if all the students shifted one chair to the left once, twice, or thrice, they would all have the same neighbors that they originally had. This means that for _each_ of the 24 ways the students can sit, 4 of those ways are not considered different. + 
...
****
.Example {counter:countexample} 
A college instructor is storing whiteboard pens in boxes. If the instructor has 96 pens and stores 12 pens in each box, how many boxes did the instructor use? 

.Solution 
There are 12 pens stored in each box and 96 pens in total, so the instructor used $\frac{96}{12} = 8$ boxes.
...
If $k+1$ pigeons are roosting in $k$ pigeonholes then at least one pigeonhole must contain more than one pigeon. + 
 + 
[small]#NOTE: The Pigeonhole Principle is often attributed to Peter Gustav Lejeune Dirichlet, who called it the _Schubfachprinzip._ The remixer is willing to speculate that this principle has been known for at least as long as humans have link:https://en.wikipedia.org/wiki/Pigeon_keeping[kept birds such as pigeons.]#
****
...
// MKD added Jan 26 2025
//The Pigeonhole Principle is a easy to justify: If you start with $k+1$ pigeons and $k$ empty pigeonholes, you can choose pigeons, one by one, to assign to each empty pigeonhole, but after you have assigned the first $k$ pigeons to their holes, there is still $1$ pigeon remaining which will need to share a pigeonhole with one of the pigeons you already chose.
