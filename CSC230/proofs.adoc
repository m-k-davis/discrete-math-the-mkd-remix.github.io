...
_This chapter was last updated on March 4, 2026_ + 
_fixed some typos_ + 
_inserted Python code for a “proof by cases” example_ + 
_inserted “proof by picture” just or fun!_ + 
//// 
_Revised example of valid and invalid arguments. Introduced new example using rules of inference with predicates._ + 
_Moved explanation of converse error using Euler diagrams to later in the chapter._ + 
_Revised example 4._ +
_Made minor wording changes and fixed typos._ + 
//// 
_NOTE: Statement that universal generalization is used in proofs by mathematical induction has been removed._ + 
...
Recall from the link:./logic.html[Logic] chapter that an *argument* is a finite sequence of statements that ends with a final statement, called the *conclusion,* that is based on inferences made from the earlier statements, called *premises* or *hypotheses.*
...
// MKD Feb 22 2025
Just as a proposition is a single statement that may be either True or False (but not both), an argument is a finite sequence of statements that may be either valid or invalid (but not both.)
...
// MKD Sep 28 2025 see https://en.wikipedia.org/wiki/Ringschluss
...
//we 
you 
use *rules of inference,* which are ways to draw a True conclusion from one or more premises that are already known to be True (or assumed to be True). That is, a *rule of inference* is an argument form that corresponds to a tautology, and so is 
a 
valid 
argument form. 
...
// MKD Sep 28 2025 revised to avoid predicates
//The following argument was used as an example in the link:./logic.html[Logic] chapter. 
The following argument is based on an example in the link:./logic.html[Logic] chapter. 
...
//“” 
This argument form is named *_modus ponens_* which is translated very roughly from Latin as “method of affirming.” 
_Modus ponens_ is a 
//*valid* 
valid 
...
//we 
you 
do not have enough information to reach the conclusion 
...
earned a degree in the new major, or perhaps there is some other possibility -  
//we 
you 
cannot determine 
//the truth value of 
...
//“” 
This argument form is an example of a *fallacy* or *_non sequitur,_* which is Latin for “it does not follow.” 
//This 
...
Notice that there is at least one row of the truth table in which both $q$ and $p \rightarrow q$ are True but $p$ is False! This means that 
//we 
you 
//can NOT 
CANNOT 
infer that $p$ is True whenever $(q \land (p \rightarrow q))$ is True. The argument form is *invalid* because $(q \land (p \rightarrow q)) \rightarrow p$ is not a tautology. 
...
Also, recall that $(p \rightarrow q) \equiv (\neg q \rightarrow \neg p)$. That is, $(p \rightarrow q) \leftrightarrow (\neg q \rightarrow \neg p)$ is a tautology. This means that the conditional is logically equivalent to its contrapositive. The conditional is NOT logically equivalent to either its converse or its inverse, as was shown using truth tables in the link:./logic.html#conditional_converse_inverse_contrapositive[Logic]chapter.
...
// MKD Sept 28 2025 this image and the text need to moved later in the chapter 
//// 
//start moved example 
...
Later in link:./proofs.html#conditional_as_subset_relation[this chapter] you will see how these four conditionals can be viewed as describing the subset relationship between two sets, which may help you recognize when either a converse error or an inverse error is being made.
...
=== An Example Using Rules Of Inference For Predicates

// MKD Sep 28 2025 revised to avoid predicates
****
.Example {counter:proofexample} - An Argument That Involves A Predicate
...
//“” 
Notice that the second proposition above involves a universally-quantified predicate: 
//The second proposition is logically equivalent to applying the quantifier “Anyone who” to the predicate “If $\rule{12mm}{.5pt}$ earned a B.S. in Computer Science then they must have earned a C or better in Discrete Math.” 
The universal quantifier “Anyone” is applied to the predicate “$\rule{12mm}{.5pt}$ who earned a B.S. in Computer Science must have earned a C or better in Discrete Math.” 
//applied to all persons in the universe of discourse, which in this case is likely to be students.
...
// proposition to complete the argument as follows. 
[no-bullet]
* Sarah earned a B.S. in Computer Science.
* Anyone who earned a B.S. in Computer Science must have earned a C or better in Discrete Math.
//* If Sarah earned a B.S. in Computer Science, then Sarah must have earned a C or better in Discrete Math. 
...
//// 
Let $P(x)$ be the predicate “$\rule{12mm}{.5pt}$ earned a B.S. in Computer Science.” + 
Let $Q(x)$ be the predicate “$\rule{12mm}{.5pt}$ earned a C or better in Discrete Math.” + 
This new argument is of the form 
[no-bullet]
* $P(\text{“Sarah”})$
* $\forall x (P(x) \rightarrow Q(x))$
* $(P(\text{“Sarah”}) \rightarrow Q(\text{“Sarah”})$ by _universal instantiation._ 
* $\therefore Q(\text{“Sarah”})$ using _modus ponens._
//// 
...
// MKD Sept 28 2025 added anchor
[[conditional_as_subset_relation]] 
== Explaining The Converse Error Using An Euler Diagram 

// MKD Sept 28-29 2025 this image and the text were moved then the text was revised 
...
//The image can also be used to explain why the inverse error is a fallacy. 

.You Try

//Describe a question that uses the two sets in the image to explain why the inverse error is a fallacy. 
...
//“” 
For each of the natural numbers 0, 1, ..., 10, the predicate $P(n)$: “$n^{2} + n + 41$ is a prime integer.” evaluates to a proposition that is True. In fact, $P(n)$ evaluates to a proposition that is True for each of the natural numbers $n$ that is less than 
// MKD Oct 6 2025 corrected inequality
//or equal to 
40. 

//“” 
It may seem that we have “checked enough cases” to conclude that $P(n)$ will evaluate to a proposition that is True for every possible natural number value of $n.$ 
// MKD Oct 6 2025 corrected inequality
//However, $P(41)$ is the proposition “$41^{2} + 41 + 41 \text{ is a prime integer}$,” which is False - notice that $41^{2} + 41 + 41 = 41 \cdot (41 + 1 + 1) = 41 \cdot 43$ is a composite number.
However, $P(40)$ is the proposition “$40^{2} + 40 + 41 \text{ is a prime integer}$,” which is False - notice that $40^{2} + 40 + 41 = 40 \cdot (40 + 1)+ 41 = 40 \cdot 41 + 41 = 41^2$ is a composite number.
...
// MKD March 13 2026 inserted anchor 
[[linear_combination_of_multiples_is_a_multiple]]
...
=== “Proof By Picture” #SECTION IN PROGRESS#


//image::imagesMKD/TriangularProofByPicture.png[TriangularProofByPicture,513,841,float="right",align="center"] 
image::imagesMKD/TriangularProofByPicture.png[TriangularProofByPicture,128,210,float="right",align="center"] 

A commonly-used technique that is not considered rigorous by many mathematicians involves drawing a picture that suggests that a proposition is True. For example, the image suggests that for every natural number $n,$  the $n$th triangular number $T_{n}$, defined to be the sum of all natural numbers that are less than of equal to $n,$ is equal to $\frac{1}{2}n(n+1).$ Notice that the picture only proves one case, that $T_{4} = 0 + 1 + 2 + 3 + 4 = \frac{1}{2}(4)(5)$ but that it seems that we should be able to produce a  picture like this for any $n$ we want. A more rigorous argument can be made using the link:./induction.html[mathematical induction] proof technique that is discussed later in the textbook.



//==== Application: Validating Code 
== Application: Validating Code 

// MKD March 1 2026 - code sample
// version 1 - do not use 
//https://pythontutor.com/visualize.html#code=def%20theSmallest%28a,b,c%29%3A%0A%20%20%20%20if%20a%20%3C%20b%3A%0A%20%20%20%20%20%20%20%20if%20a%20%3C%20c%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20a%0A%20%20%20%20%20%20%20%20%23else%3A%0A%20%20%20%20%20%20%20%20%23%20%20%20%20return%20c%0A%20%20%20%20else%3A%20%23%20b%20%3C%3D%20a%0A%20%20%20%20%20%20%20%20if%20b%20%3C%20c%3A%20%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20b%0A%20%20%20%20return%20c%0A%20%20%20%20%0Aprint%28%20theSmallest%285,%203,%202%29%20%29&cumulative=false&curInstr=6&heapPrimitives=nevernest&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false
// version 2 
//https://pythontutor.com/visualize.html#code=def%20theSmallest%28a,b,c%29%3A%0A%20%20%20%20if%20a%20%3C%20b%3A%0A%20%20%20%20%20%20%20%20if%20a%20%3C%20c%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20a%0A%20%20%20%20elif%20b%20%3C%20c%3A%20%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20b%0A%20%20%20%20return%20c%0A%20%20%20%20%0Aprint%28%20theSmallest%285,%203,%202%29%20%29&cumulative=false&curInstr=7&heapPrimitives=nevernest&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false
// Example should ask "Explain why theSmallest will return the least value of the numbers a, b, and c." 
//version 3 - use this one 
//https://pythontutor.com/visualize.html#code=def%20minimumOfThree%28a,b,c%29%3A%0A%20%20%20%20if%20a%20%3C%20b%3A%0A%20%20%20%20%20%20%20%20if%20a%20%3C%20c%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20a%0A%20%20%20%20else%3A%0A%20%20%20%20%20%20%20%20if%20b%20%3C%20c%3A%20%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20b%0A%20%20%20%20return%20c%0A%20%20%20%20%0Aprint%28%20minimumOfThree%285,%203,%202%29%20%29&cumulative=false&curInstr=7&heapPrimitives=nevernest&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false


:pt_code_height: 450 
:pt_rawinput: false
:pt_code: def%20minimumOfThree%28a,b,c%29%3A%0A%20%20%20%20if%20a%20%3C%20b%3A%0A%20%20%20%20%20%20%20%20if%20a%20%3C%20c%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20a%0A%20%20%20%20else%3A%0A%20%20%20%20%20%20%20%20if%20b%20%3C%20c%3A%20%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20b%0A%20%20%20%20return%20c%0A%20%20%20%20%0Aprint%28%20minimumOfThree%285,%203,%202%29%20%29


As an application of the 
// “proof by cases” 
proof techniques, consider the following code. 

****
.Example {counter:proofexample} - Code validation
--
--

Does the function _minimumOfThree_ actually return the minimum of the three input numbers?

include::python_tutor.adoc[]

****

//You can use the _Next_ button to step through the code. 

Notice that in the example above, you could either 
(1) use logic and the “proof by cases” technique to make an argument that _minimumOfThree_ must work as its name indicates, or 
(2) use “proof by cases” thinking to create different test cases for the values assigned to the three variables $a,$ $b,$ and $c,$ then evaluate the function at those test cases to try to validate that the function works as its name indicates.
