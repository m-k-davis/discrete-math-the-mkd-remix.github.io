...
_This chapter was last updated on April 26, 2026._ + 
_Inserted brief discussion of the Halting Problem._ + 
_Inserted a brief section to mention the master theorem and the Akra–Bazzi method._ +
_Corrected text in Euclidean Algorithm example._ +
_added closed-form solution for the recurrence relation that describes the number of comparisons used by the MergeSort algorithm for lists of length a power of 2._ + 
//_updated discussion of the MergeSort algorithm._ + 
//_inserted content on Dijkstra's algorithm._ + 
//[small]#Contents locked until 11:59 p.m. Pacific Standard Time on May 23, 2025.#


// MKD Sep 7 2025 
// link to my Russian Peasant Multiplication implementation 
//	https://pythontutor.com/render.html#code=def%20RPM%28a,b%29%3A%0A%20%20%20%20result%20%3D%200%0A%20%20%20%20if%20b%20%3D%3D%201%3A%20return%20a%0A%20%20%20%20while%20b%20%3E%201%3A%0A%20%20%20%20%20%20%20%20if%20b%252%3A%20%23%20if%20b%20is%20odd%3A%20%0A%20%20%20%20%20%20%20%20%20%20%20%20result%20%2B%3D%20a%0A%20%20%20%20%20%20%20%20a%20%3D%20a%20%3C%3C%201%0A%20%20%20%20%20%20%20%20b%20%3D%20b//2%20%23%20b%20%3D%20math.floor%28b/2%29%0A%20%20%20%20result%20%2B%3D%20a%0A%20%20%20%20return%20result%0A%0Aprint%28RPM%2897,%2013%29%29&cumulative=false&curInstr=23&heapPrimitives=nevernest&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false
// not currently used in the book, but used in a classroom activity.
...
[small]#The word _algorithm_ is derived from the name of link:https://www.britannica.com/biography/al-Khwarizmi[Abū ‘Abd Allāh Muḥammad ibn Mūsā al-Khwārizmī.] Western Europeans learned how to do arithmetic with Hindu-Arabic numerals and the base-ten place-value system from a 12th-century AD Latin translation of one of al-Khwārizmī's books 
//written in Arabic. 
//(The original Arabic book no longer exists, but was about 300 years older than the Latin translation.) 
...
By the way, the word _algebra_ is derived from the title of another one of Al-Khwārizmī's books.# 
//to achieve a specified task producing a determined output. + 
//Note that ; however, link:https://en.wikipedia.org/wiki/Principles_of_Hindu_Reckoning[other Arabic writings] still exist. 
// and arithmetic is the basis of modern decimal notation. 
// MKD: Brittanica names him Muḥammad ibn Mūsā al-Khwārizmī
// MKD: Knuth names him Abū ‘Abd Allāh Muḥammad ibn Mūsā al-Khwārizmī


// MKD moved Knuth's definition into a new section


////
// MKD algorithm list begins
Examples of common discrete mathematics algorithms include:

* Arithmetic algorithms such as the integer division or computation of the greatest common divisor of two integers. 
// presented earlier in the textbook. 
...
Additional algorithms that involve graphs and trees will be introduced later in this textbook.
// MKD algorithm list ends
////



//MKD added the following
NOTE: In this chapter, several algorithms are implemented in Python using non-optimal code to illustrate all steps needed. 
//Most of the Python examples in the Remix use non-optimal code. 
//As mentioned in the link:./index.html[About this text] chapter, this was a deliberate choice because the code examples are designed for _teaching the mathematical concepts_ but not for illustrating the link:https://docs.python.org/3/glossary.html#term-Pythonic["Pythonic"] way of coding. However, a few code examples in this chapter _do_ show how you can complete the same task by using some of Python's built-in methods.
// MKD Apr 1 2025
As mentioned in the link:./index.html[About this text] chapter, this was a deliberate choice because the code examples are designed for _teaching the mathematical concepts_ and for neither presenting the most efficient implementations nor illustrating the link:https://docs.python.org/3/glossary.html#term-Pythonic["Pythonic"] way of coding. However, a few code examples in this chapter _do_ show how you can complete the same task by using some of Python's built-in methods.
...
== What _Is_ An Algorithm? What Is _Not_ An Algorithm?

There are several link:https://opened.cuny.edu/courseware/lesson/46/student/?section=2[problem-solving strategies] that humans use, but not all of these strategies are algorithms. 

// MKD Knuth definition begins
//More precisely, an algorithm should have the following properties, which the remixer has paraphrased from Donald E. Knuth's famous link:https://learning.oreilly.com/library/view/art-of-computer/9780321635754[_The Art of Computer Programming, The: Volume 1: Fundamental Algorithms, 3rd Edition_]:
An algorithm should have the following properties, based on Donald E. Knuth's description in his famous  link:https://learning.oreilly.com/library/view/art-of-computer/9780321635754[_Art of Computer Programming, The: Volume 1: Fundamental Algorithms, 3rd Edition_]:

//Art of Computer Programming, The: Volume 1: Fundamental Algorithms, 3rd Edition by Donald E. Knuth
...
Output:: The algorithm produces one or more outputs that have a specified relation to the inputs. Furthermore, the algorithm should produce the _correct_ outputs for each set of inputs.

////
* Definiteness. Each step must be precisely defined. The actions taken at each step must be unambiguously specified. 
* Input. Zero or more input quantities are taken from a specified set. The inputs can be introduced before the initial step of the algorithm or dynamically before any later step of the algorithm.
* Finiteness: The process must terminate after a finite number of steps (although the number of steps may be a very large natural number.) 
* Effectiveness. Each step must be so basic that it can be done exactly and in a finite length of time. 
* Output. The algorithm produces one or more outputs that have a specified relation to the inputs. Moreover, the algorithm should produce the _correct_ outputs for each set of inputs.
////
// MKD Knuth definition ends


//=== Find Minimum in a List of Integers
=== An Example: Finding the Minimum in a List of Integers

//As an example, consider an algorithm to determine the minimum element in a finite sequence of integers.

//The algorithm could be constructed as follows:
...
** Input: A finite list of integers _L._
** Steps:  
...
:pt_code_height: 600
:pt_rawinput: false
:pt_code: def%20minimum%28a%29%3A%0A%20%20%20%20n%20%3D%20len%28a%29%0A%20%20%20%20if%20%28n%3D%3D0%29%3A%0A%20%20%20%20%20%20%20%20return%20None%0A%20%20%20%20min%20%3D%20a%5B0%5D%0A%20%20%20%20i%20%3D%201%0A%20%20%20%20while%20%28i%20%3C%20n%29%3A%20%0A%20%20%20%20%20%20%20%20if%20%28a%5Bi%5D%20%3C%20min%29%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20min%20%3D%20a%5Bi%5D%0A%20%20%20%20%20%20%20%20i%20%3D%20i%20%2B%201%0A%20%20%20%20return%20min%0A%0AL%20%3D%20%5B3,%202,%201,%205,%20-4,%206%5D%0Am%20%3D%20minimum%28L%29%0Aprint%28%22The%20minimum%20value%20in%22,L,%22is%22,str%28m%29%2B'.'%29

// OLD CODE def%20minimum%28a%29%3A%0A%20%20%20%20min%20%3D%20a%5B0%5D%0A%20%20%20%20for%20i%20in%20range%280,%20len%28a%29%29%3A%0A%20%20%20%20%20%20%20%20if%20a%5Bi%5D%20%3C%20min%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20min%20%3D%20a%5Bi%5D%0A%20%20%20%20return%20min%0A%0Am%20%3D%20minimum%28%5B3,%202,%201,%205,%20-4,%206%5D%29%0Aprint%28%22The%20minimum%20is%22,%20m%29
...
Historically, algorithms first arose as link:https://en.wikipedia.org/wiki/Algorithm#History[ways to solve arithmetic problems.] In this section, algorithms for such operations are presented. 
...
//=== Division Of Natural Numbers By Repeated Division 
=== Division By Repeated Subtraction 

//In this subsection, two algorithms for division are presented.
A simple division algorithm is presented in this subsection. 

:pt_code_height: 500
:pt_rawinput: false
:pt_code: a%20%3D%20130%0Ab%20%3D%203%0A%23%20Initialize%20output%20variables%20and%20run%20the%20loop%0Aq%20%3D%200%0Ar%20%3D%20a%0Aprint%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%0Awhile%20not%20%28%20b%3Er%20and%20r%3E%3D0%20%29%3A%0A%20%20%20%20q%20%3D%20q%20%2B%201%0A%20%20%20%20r%20%3D%20r%20-%20b%0A%20%20%20%20print%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%20%23%20intermediate%20result%0Aprint%28%22Dividing%22,a,%22by%22,b,%22gives%20quotient%22,q,%22and%20remainder%22,str%28r%29%2B'.'%29

// MKD version for arbitrary integers a and b
//a%20%3D%2013%0Ab%20%3D%20-3%0A%23%20Initialize%20output%20variables%20and%20run%20the%20loop%0Aq%20%3D%200%0Ar%20%3D%20a%0Aprint%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%0Awhile%20not%20%28%20abs%28b%29%3Er%20and%20r%3E%3D0%20%29%3A%0A%20%20%20%20if%20a*b%3C%200%3A%0A%20%20%20%20%20%20%20%20q%20%3D%20q%20-%201%0A%20%20%20%20%20%20%20%20r%20%3D%20r%20%2B%20b%0A%20%20%20%20else%3A%0A%20%20%20%20%20%20%20%20q%20%3D%20q%20%2B%201%0A%20%20%20%20%20%20%20%20r%20%3D%20r%20-%20b%0A%20%20%20%20print%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%20%23%20intermediate%20result%0Aprint%28%22Dividing%22,a,%22by%22,b,%22gives%20quotient%22,q,%22and%20remainder%22,str%28r%29%2B'.'%29

//a%20%3D%20-13%0Ab%20%3D%203%0A%23%20Initialize%20output%20variables%20and%20run%20the%20loop%0Aq%20%3D%200%0Ar%20%3D%20a%0Aprint%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%0Awhile%20not%20%28%20b%3Er%20and%20r%3E%3D0%20%29%3A%0A%20%20%20%20if%20a%20%3C%200%3A%0A%20%20%20%20%20%20%20%20q%20%3D%20q%20-%201%0A%20%20%20%20%20%20%20%20r%20%3D%20r%20%2B%20b%0A%20%20%20%20else%3A%0A%20%20%20%20%20%20%20%20q%20%3D%20q%20%2B%201%0A%20%20%20%20%20%20%20%20r%20%3D%20r%20-%20b%0A%20%20%20%20print%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%20%23%20intermediate%20result%0Aprint%28%22Dividing%22,a,%22by%22,b,%22gives%20quotient%22,q,%22and%20remainder%22,str%28r%29%2B'.'%29%0A

//a%20%3D%2013%0Ab%20%3D%203%0A%23%20Initialize%20output%20variables%20and%20run%20the%20loop%0Aq%20%3D%200%0Ar%20%3D%20a%0Aprint%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%0Awhile%20not%20%28%20b%3Er%20and%20r%3E%3D0%20%29%3A%0A%20%20%20%20q%20%3D%20q%20%2B%201%0A%20%20%20%20r%20%3D%20r%20-%20b%0A%20%20%20%20print%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%20%23%20intermediate%20result%0Aprint%28%22Dividing%22,a,%22by%22,b,%22gives%20quotient%22,q,%22and%20remainder%22,str%28r%29%2B'.'%29%0A
...
****
.Example {counter:algoex} - Division of Integers by Repeated Subtraction
--
--
The code below implements integer division of the positive integer _a_ by the positive integer _b._ This algorithm is very ancient, dating back to at least link:https://en.wikipedia.org/wiki/Euclidean_division#History[Euclid's _Elements._]
...
Notice that the "division by repeated subtraction" algorithm has a worst-case scenario when the divisor _b_ equals 1. 
//If _n_ is the number _a,_ then 
//It should be clear that "division by repeated subtraction" is $O(a),$ that is, of order less than the value of the dividend $a,$ since in the worst-case scenario you must subtract the divisor $b=1$ exactly $a$ times to exit the loop.
// MKD Apr 14 2025 
In this worst-case scenario you must subtract the divisor $b=1$ exactly $a$ times to exit the loop, so "division by repeated subtraction" is $O(a),$ that is, of at least linear order in the larger number $a.$


=== Long Division Of Natural Numbers
// MKD insert example using "binary" shifts
You can revise the previous algorithm to work faster by using place-value thinking.

:pt_code_height: 650
:pt_rawinput: false
:pt_code: a%20%3D%20130%0Ab%20%3D%203%0A%23%20Initialize%20output%20variables%20and%20run%20the%20loop%0Aq%20%3D%200%0Ar%20%3D%20a%0Apower_of_2%20%3D%201%0Ab_times_power_of_2%20%3D%20b%0Aprint%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%0Awhile%28a%20%3E%20b_times_power_of_2%29%3A%20%23%20shift%20b%20to%20correct%20%22binary%20column%22%0A%20%20%20%20power_of_2%20%3D%20power_of_2%20%3C%3C%201%0A%20%20%20%20b_times_power_of_2%20%3D%20b_times_power_of_2%20%3C%3C%201%0Awhile%28r%3Eb%29%3A%20%23%20subtract%20down%20%22binary%20columns%22%0A%20%20%20%20if%28r%3E%3Db_times_power_of_2%29%3A%0A%20%20%20%20%20%20%20%20q%20%3D%20q%20%2B%20power_of_2%0A%20%20%20%20%20%20%20%20r%20%3D%20r%20-%20b_times_power_of_2%0A%20%20%20%20%20%20%20%20print%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%20%23%20intermediate%20result%0A%20%20%20%20power_of_2%20%3D%20power_of_2%20%3E%3E%201%0A%20%20%20%20b_times_power_of_2%20%3D%20b_times_power_of_2%20%3E%3E%201%20%0Aprint%28%22Dividing%22,a,%22by%22,b,%22gives%20quotient%22,q,%22and%20remainder%22,str%28r%29%2B'.'%29

//a%20%3D%2013%0Ab%20%3D%203%0A%23%20Initialize%20output%20variables%20and%20run%20the%20loop%0Aq%20%3D%200%0Ar%20%3D%20a%0Apower_of_2%20%3D%201%0Ab_times_power_of_2%20%3D%20b%0Aprint%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%0A%23%20shift%20b%20over%20to%20correct%20column...%20but%20as%20done%20in%20binary%20notation!%0Awhile%28a%20%3E%20b_times_power_of_2%29%3A%0A%20%20%20%20power_of_2%20%3D%20power_of_2%20*%202%0A%20%20%20%20b_times_power_of_2%20%3D%20b_times_power_of_2%20*%202%0Apower_of_2%20%3D%20power_of_2%20%3E%3E%201%20%23%20floor%20division%20by%202%0Ab_times_power_of_2%20%3D%20b_times_power_of_2%20%3E%3E%201%0A%23%20now%20subtract%20down%20columns%20as%20in%20long%20division%20with%20decimals%0Awhile%28r%3Eb%29%3A%0A%20%20%20%20if%28r%3E%3Db_times_power_of_2%29%3A%0A%20%20%20%20%20%20%20%20q%20%3D%20q%20%2B%20power_of_2%0A%20%20%20%20%20%20%20%20r%20%3D%20r%20-%20b_times_power_of_2%0A%20%20%20%20%20%20%20%20print%28a,%22%3D%22,q,%22*%22,b,%22%2B%22,r%29%20%23%20intermediate%20result%0A%20%20%20%20power_of_2%20%3D%20power_of_2%20%3E%3E%201%0A%20%20%20%20b_times_power_of_2%20%3D%20b_times_power_of_2%20%3E%3E%201%20%0Aprint%28%22Dividing%22,a,%22by%22,b,%22gives%20quotient%22,q,%22and%20remainder%22,str%28r%29%2B'.'%29


****
//.Example {counter:algoex} - Long Division using Base-Two Shifts But Base-Ten Notation
//.Example {counter:algoex} - Division of Integers (Long Division)
.Example {counter:algoex} - Long Division
--
--
The code below implements integer division of the integer _a_ by the _positive integer_ _b._
...
[small]#Notice that the shift operators `<<` and `>>` multiply or divide by 2 (`>>` gives only the quotient, without keeping track of the remainder.)#
...
// MKD Apr 14 2025 
The worst-case scenario corresponds to the divisor $b=1,$ which requires you to shift as many times as there are _binary_ digits in $a.$ Notice that $a$ is bounded above by 2 raised to the number of binary digits; this means that 
//if _n_ is the number _a,_ this 
the long division algorithm is $O(\log (a)).$ + 




NOTE: Python provides built-in operators to find the quotient and remainder for variables `a` and `b` of type `int`: `a//b` is the quotient and `a%b` is the remainder. There is also the operator `divmod(a,b)` that returns an ordered pair (of data type `tuple`) containing both the quotient and the remainder, but `divmod` is less efficient than `//` and `%` and should only be used when you need to find both the quotient and the remainder.


// MKD insert "Pythonic" example 


[[algorithms_euclidean_algorithm]]
=== Greatest Common Divisors: The Euclidean Algorithm 

It is often needed to find a common divisor of two integers. This next algorithm goes back to link:http://aleph0.clarku.edu/~djoyce/elements/bookVII/propVII1.html[Euclid.]

:pt_code_height: 500
:pt_rawinput: false
:pt_code: %23%20m%20and%20n%20are%20positive%20integers%0Am%20%3D%20360%0An%20%3D%20210%0A%23%20remainder%20when%20m%20is%20divided%20by%20r%0A%23r%20%3D%20m%25n%20%0Aq,r%20%3D%20divmod%28m,n%29%0Aprint%28m,'%3D',q,'*',n,'%2B',r%29%0Awhile%20%28r%20!%3D%200%29%3A%0A%20%20%20%20m%20%3D%20n%0A%20%20%20%20n%20%3D%20r%0A%20%20%20%20%23r%20%3D%20m%25n%0A%20%20%20%20q,r%20%3D%20divmod%28m,n%29%0A%20%20%20%20print%28m,'%3D',q,'*',n,'%2B',r%29%0Aprint%28n%29%0A

****
.Example {counter:algoex} - The Euclidean Algorithm
--
--
The code below implements the Euclidean algorithm that is used to compute the greatest common divisor of two _positive_ integers _m_ and _n._ 
...
The Euclidean Algorithm takes two positive integer inputs $a$ and $b$ with $a > b.$ It can be proven using link:./induction.html[mathematical induction] that the worst-case scenario for this algorithm is link:https://en.wikipedia.org/wiki/Euclidean_algorithm#Worst-case[$O(\log (b)).$] The proof uses the closed-form for the Fibonacci numbers.

NOTE: Python's `math` module includes a function `gcd` that will compute the greatest common divisor of two or more integers.
...
#*RECOMMENDATION:* The link:https://calearninglab.org/project/expanding-equity-and-access-in-discrete-mathematics/["Algorithms and Recursive Functions" activity] can replace one or both of the first two subsections.#
...
Negative integers can be _valid indices_ for a Python list! This is very different than other languages like Java in which indices [underline]#must# be natural numbers. 
...
If you need to know whether the target value _x_ is in the list _s_ but do not need the least index, you can use `x in s` which returns a `Boolean.`
...
If you need to know how many times the target value _x_ occurs in the list _s_ you can use the `list.count(x)` method.
...
// MKD Apr 01 2025
[small]#In fact, most modern programming languages have built-in sort methods for you to use; these built-in methods are implemented using efficient algorithms.# 
...
// MKD Apr 01 2025
You can use link:./induction.html[mathematical induction] to prove that \[1+2+\cdots+(n-2)+(n-1)= \frac{(n-1)\cdot n}{2}\] and since $\frac{(n-1)\cdot n}{2} =\frac{1}{2}n^2-\frac{1}{2}n$, the bubble sort algorithm is $O(n^2)$.
...
// MKD Nov 1 2025 inserted anchor
[[merge_sort]]
...
[small]#Image credit: link:https://commons.wikimedia.org/wiki/File:Merge_sort_algorithm_diagram.svg["Merge Sort Algorithm Diagram"] by VineetKumar. This work has been released into the public domain by its author, VineetKumar at English Wikipedia. This applies worldwide.# 
[.float-group]
--
...
// MKD Nov 10 2025 new explanation
...
[small]#Informal exercise: Verify that $C(n) = n \cdot \log_{2}{(n)} - (n-1)$ is a closed-form solution of the simplified recurrence relation.#

// MKD Mar 27 2026 see also Akra–Bazzi method
//https://en.wikipedia.org/wiki/Akra%E2%80%93Bazzi_method#Significance

// MKD Mar 27 2026 - need to add note on Master Theorem
// https://en.wikipedia.org/wiki/Master_theorem_(analysis_of_algorithms)
// https://apps.dtic.mil/sti/tr/pdf/ADA064294.pdf (Bentley, Haken, and Saxe)

//==== Big-O Analysis of Divide-and-Conquer Algorithms 
== Big-O Analysis of Divide-and-Conquer Algorithms 

The Merge Sort is 
//an 
just one 
example of a *divide-and-conquer algorithm* that 
breaks up a large problem into two or more smaller versions of the same problem and then combines the solutions of the smaller problems to find a solution to the original large problem. 
...
//can be described by a recurrence relation of the form \[ T(n) = k T \left( \frac{n}{c} \right) + f(n)\] where
can be described by a recurrence relation of the form \[ T(n) = a T \left( \frac{n}{b} \right) + f(n)\] where
//where $k$ and $c$ are positive integer constants, 
////
where the constants $k$ and $c$ stand for positive integers, 
//and 
//$f$ is a known function, 
either the function $f$ or its $\Theta(f)$ class is known, 
//and 
//(and as in the preceding discussion, 
the input values for $n$ are restricted to powers of $2,$ 
//for the sake of simplicity.) 
and a basis for the recurrence (e.g., the value of $T(1)$) is given. 
//+ 
//[small]#In some cases, only $\Theta(f)$ is known.#
////
//* $k$ and $c$ stand for positive integer constants, 
* $a$ and $b$ stand for positive integer constants, 
* either the function $f$ or its $\Theta(f)$ class is known, 
* the input values for $n$ are restricted to powers of the constant base $b,$ and 
//* a basis for the recurrence relation (e.g., the value of $T(1)$) is given. 
* a basis for the recurrence relation is given, e.g., the value of $T(1)$ is known. 


//It is worth mentioning that there are many other “divide-and-conquer” algorithms whose time complexity can be described by a recurrence relation of the form 
//There 

//In general, you only need to determine the big $O$ or big $\Theta$ of the function $T,$ that is, you don't need to find a closed form for the function $T.$ 
//In general, you only need to care about the big $\Theta$ or big $O$ of the function $T,$ 
////
In general, you only need to determine the big $\Theta$ 
//or big $O$ 
of the function $T,$ 
//that is, you don't need to find a 
which is easier to do than finding a closed form for the function $T.$ 
//but can instead use a method such as 
//In many cases, you can use the 
//You can use 
//methods such as 
////
In general, you don't need to find a closed form for the function $T$ (which may be difficult) but only to determine the big Theta of the function $T,$  
which is easier to do. 

//To determine the big $\Theta$ of $T,$ you can use
To determine $\Theta(T),$ you can use
the  link:https://en.wikipedia.org/wiki/Master_theorem_(analysis_of_algorithms)[“master theorem”] originally described by Bentley, Haken, and Saxe (link:https://apps.dtic.mil/sti/tr/pdf/ADA064294.pdf[pdf link.]) 
//Alternatively, you can use 
//or 
Alternatively, you can use 
the 
link:https://en.wikipedia.org/wiki/Akra%E2%80%93Bazzi_method#Significance[Akra–Bazzi method] 
//to find the big $\Theta$ of $T.$
which works for this kind of recurrence relation as well as ones that are even more complicated. 
...
In the second part of Merge Sort, $n$ elements are merged which is $O(n)$. Since the algorithm performs the first and the second parts, we can 
multiply to find that the complexity of Merge Sort is $O(n \log{n})$.
////
...
// MKD Apr 20 2026  
//== Can EVERY Problem be Solved Using An Algorithm? #section in progress# 
//== Can Algorithms Solve Every Possible Problem? #section in progress# 
//== Can Algorithms Solve Every Possible Problem? #section in progress# 
== A Brief Discussion of the Halting Problem #section in progress# 

It would be 
//nice 
ideal 
if _every_ problem could be solved using some algorithm, but Alan Turing proved in 1936 that this cannot be True. 
//there are problems that cannot be solved. 
In this section, we will look at an informal proof that the Halting Problem cannot be solved by any algorithm. 

//The Halting Problem is the the following statement. 
.The Halting Problem
****
For any given algorithm $A$ along with input $I$ for $A$, decide if the algorithm eventually halts (i.e., finishes running) or runs forever for the input $I.$ 
****
// a description of an arbitrary
//Given any algorithm $A$ (e.g., a computer program) and its input $I$, decide if the algorithm will eventually finish running or if it will run forever. 
//Given any algorithm $A$ and input $I$ for $A$, decide if the algorithm  eventually halts (i.e., finishes running) or runs forever for the input $I.$ 
We seek an algorithm $P$ that solves the Halting Problem. That is, the algorithm $P$ must work for all possible ordered pairs $( A, \, I )$ where $A$ is an algorithm and $I$ is some input to $A.$ 
We describe such an algorithm below.

//Find an algorithm $P$ that can perform the following task: 
//Consider the following algorithm that we will call $P.$ 
//Let's _assume_ that we can find an algorithm $P$ that satisfies the following description. 
//Consider the algorithm $P$ that satisfies the following description. 
//Consider the algorithm $P$ described below. 


//._P, the Decider_ 
._P_ 
** Task: Given an algorithm $A$ and input $I$ for $A$, decide whether $A$ eventually halts or runs forever for the input $I.$ 
** Input: An ordered pair $( A, \, I )$ where $A$ is an algorithm (e.g., source code) and $I$ is input for $A.$  
//** Steps:  
//. (We don't know the steps - we are trying to find out if they even exist!!)
//** Steps: We don't know these yet. In fact, we are trying to find out if they even exist!!
** Steps: We don't know these yet. In fact, we are trying to figure out what they could be!
//** Output: $P$ returns “Yes” if $A$ halts for input $I,$ and $P$ returns “No” if $A$ runs forever for input $I.$  
** Output: “Yes” if $A$ halts for input $I,$ or “No” if $A$ runs forever for input $I.$  

Notice that if we can figure out how to fill in the Steps,
// If such a $P$ actually exists, it 
then $P$ 
is a solution to the Halting Problem. + 
That is, $P(A, I)$ returns “Yes” if and only if $A$ halts for input $I,$ and 
$P(A, I)$ returns “No” if and only if $A$ runs forever for input $I.$ + 
[small]#Note: If it helps, you can imagine that $P$ is a program that, like a compiler or debugger, takes in source code or a compiled binary for program $A$ and input $I$ then executes some steps to determine whether $A$ running on input $I$ halts (“Yes”) or runs forever (“No”.)# 

//and outputs either “Yes” or “No” as described above.  

We will now use a “proof by contradiction” argument to show that no such $P$ can exist! 

To get the contradiction, first we assume that such a $P$ does exist. 

Secondly, we create another algorithm $Q,$ based on $P,$ described below.

//._Q, the “Diagonalizer”_ 
._Q_ 
//** Task: Given an algorithm $A$, $Q$ decides whether $A$ eventually halts or runs forever when $A$ is given itself as its input. 
** Task: Given an algorithm $A$, halt if $A$ runs forever when $A$ is given _itself_ as its input, or run an infinite loop if $A$ eventually halts when $A$ is given _itself_ as its input. 
** Input: An algorithm $A$ (e.g., source code.) 
////
** Steps:  
. Set string $s$ to the output $P(A, A)$
. If s equals “Yes” 
...
Note that $Q(A)$ runs forever if and only if $A$ halts for input $A.$ That is, $Q(A)$ does the “opposite” of what $P(A, \, A)$ indicates.

Thirdly, we now run $Q$ _on itself as input!_ 
This means that $Q(Q)$ runs forever if and only if $Q$ halts for input $Q,$ which is a contradiction! 

Now, it should be clear that $Q$ exists if $P$ exists, but the existence of $Q$ gives us a contradiction. Therefore, it also must be the case that algorithm $P$ does not exist!

 

//For each ordered pair input $( A, \, I ),$ where $A$ is an algorithm and $I$ is input for $A,$ the algorithm $P$ outputs "Yes" whenever $A$ eventually halts when processing $I$ or prints "No" if $A

//For a alternate proof that the Halting Problem is undecidable, 
For an alternative explanation of the proof, you can read link:https://www.lel.ed.ac.uk/~gpullum/loopsnoop.html[this poem] by Geoffrey K. Pullum.

More information about the Halting Problem can be found at link:https://en.wikipedia.org/wiki/Halting_problem#Background[this Wikipedia page.]


// start of Dijkstra
//// 
// MKD Nov 9 2025 new section
== Graph Algorithms 

In this section, we will examine just one of the many algorithms 
//: Dijkstra's algorithm
//There are many algorithms 
that are used with link:./graph_theory.html[Graphs] and link:./trees.html[Trees,] namely, Dijkstra's algorithm for determining a path between two vertices of a graph that has “minimal total weight,” which will be explained in the following example. 

//In this section, we will look at just one algorithm: Dijkstra's algorithm

// MKD Nov 9 2025 Dijkstra's algorithm code - just the graph
//https://pythontutor.com/render.html#code=myVertexList%20%3D%20%5B'a',%20'b',%20'c',%20'x',%20'y',%20'z'%5D%0AmyEdgeList%20%3D%20%5B%20%0A%20%20%20%20%5B%7B'a','b'%7D,%203%5D,%20%20%0A%20%20%20%20%5B%7B'a','c'%7D,%204%5D,%20%20%0A%20%20%20%20%5B%7B'b','c'%7D,%202%5D,%20%20%0A%20%20%20%20%5B%7B'b','x'%7D,%206%5D,%20%20%0A%20%20%20%20%5B%7B'b','y'%7D,%205%5D,%20%20%0A%20%20%20%20%5B%7B'c','y'%7D,%207%5D,%20%20%0A%20%20%20%20%5B%7B'x','z'%7D,%203%5D,%20%20%0A%20%20%20%20%5B%7B'y','z'%7D,%209%5D%20%20%0A%20%20%20%20%5D%0A%23myGraph%20%3D%20%7B'a'%3A%5B%20%5B'b',3%5D,%20%5B'c',4%5D%20%5D%7D&cumulative=false&curInstr=0&heapPrimitives=false&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false 

// temp code
//https://pythontutor.com/render.html#code=myVertexList%20%3D%20%5B'a',%20'b',%20'c',%20'x',%20'y',%20'z'%5D%0AmyEdgeList%20%3D%20%5B%20%0A%20%20%20%20%5B%7B'a','b'%7D,%203%5D,%20%20%0A%20%20%20%20%5B%7B'a','c'%7D,%204%5D,%20%20%0A%20%20%20%20%5B%7B'b','c'%7D,%202%5D,%20%20%0A%20%20%20%20%5B%7B'b','x'%7D,%206%5D,%0A%20%20%20%20%5B%7B'b','y'%7D,%205%5D,%20%20%0A%20%20%20%20%5B%7B'c','y'%7D,%207%5D,%20%20%0A%20%20%20%20%5B%7B'x','z'%7D,%203%5D,%20%20%0A%20%20%20%20%5B%7B'y','z'%7D,%209%5D%20%20%0A%20%20%20%20%5D%0A%23myGraph%20%3D%20%5BmyVertexList,%20myEdgeList%5D%0AV_temp%20%3D%20myVertexList.copy%28%29%0AnumEdges%20%3D%20len%28myEdgeList%29%0AV_chosen%20%3D%20%5B%5D%20%0AV_candidates%20%3D%20%5B%5D%20%0AE_chosen%20%3D%20%5B%5D%20%0AE_candidates%20%3D%20%5B%5D%20%0AV_chosen.append%28'a'%29%0AV_temp.remove%28'a'%29%20%0Awhile%20not%20'z'%20in%20V_chosen%3A%0A%20%20%20%20v%20%3D%20V_chosen%5B-1%5D%20%23%20last%20vertex%20in%20V_chosen%0A%20%20%20%20for%20i%20in%20range%28numEdges%29%3A%0A%20%20%20%20%20%20%20%20for%20w%20in%20myVertexList%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20if%20v%20in%20myEdgeList%5Bi%5D%5B0%5D%20and%20w%20in%20myEdgeList%5Bi%5D%5B0%5D%20and%20not%20w%20in%20V_chosen%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20pass%0A%20%20%20%20V_chosen.append%28'z'%29%0A%20%20%20%20V_temp.remove%28'z'%29&cumulative=false&curInstr=137&heapPrimitives=false&mode=display&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false

****
.Example {counter:algoex} - A weighted graph
--
--

Consider the graph shown that has $6$ vertices and $8$ edges. 

//image::imagesMKD/WeightedGraphForDjikstra_v1.png[WeightedGraphForDjikstra_v1,572,265,float="right",align="center"]
//image::imagesMKD/WeightedGraphForDjikstra_v1.png[WeightedGraphForDjikstra_v1,286,133,float="right",align="center"]
image::imagesMKD/WeightedGraphForDjikstra_v1.png[WeightedGraphForDjikstra_v1,382,174,float="center",align="center"]

Notice that each edge of this graph is labeled by a number, called the *weight* of its edge. 
// The weight which represents a weight that is applied to that edge. Depending on the context, the weight may represent a physical distance between the edge's endpoints, or it may represent the cost of moving or sending something between the edge's endpoints, or it may represent the time needed to travel between the two edge's endpoints. 

A problem that naturally arises is to find a path, made by joining multiple edges, that connects two specified vertices so that the path has the minimal total weight for all such possible paths in the graph. 
...
// MKD Mar 27 2026 - commented out next line
//MORE TO COME!
