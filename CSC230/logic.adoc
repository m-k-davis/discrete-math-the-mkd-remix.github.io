...
_This chapter was last updated on February 23, 2026._ + 
_Edited section on logic circuits._
//_Added link to DNF and CNF website_ + 
...
// MKD Feb 23 2026 inserte two "the"s 
Among the applications of logic to computer science are the design of electronic circuits and the validation of algorithms and programs.

//“”

Key terms and concepts covered in this chapter:

//MKD Feb 13 2025 - cleaned up topics and terminolgy list
...
//**** how implication and equivalence are different
// MKD Feb 13 2025 added Satisfiability, tautology, and contradiction
** Truth tables
//*** how negation and contradiction are different
// MKD Feb 13 2025 - substituted wff for "validity" and "validity of well-formed formulas" following two
** Well-formed formulas
** Satisfiability, tautology, and contradiction
** Normal forms (conjunctive and disjunctive)
// MKD Feb 13 2025 - Inference is covered in the next chapter
* Predicate logic
...
// MKD new code for material implication Feb 13 2025
// https://pythontutor.com/visualize.html#code=def%20material_implication%28p,q%29%3A%0A%20%20%20%20if%20p%3A%0A%20%20%20%20%20%20%20%20if%20q%3A%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20True%0A%20%20%20%20%20%20%20%20else%3A%20%0A%20%20%20%20%20%20%20%20%20%20%20%20return%20False%0A%20%20%20%20else%3A%0A%20%20%20%20%20%20%20%20return%20True%0A%0Aprint%28%22p%20%20%20%20q%20%20%20%20p-%3Eq%22%29%0Afor%20p%20in%20%5BTrue,%20False%5D%3A%0A%20%20%20%20for%20q%20in%20%5BTrue,%20False%5D%3A%0A%20%20%20%20%20%20%20%20%23print%28p,%20q,%20material_implication%28p,%20q%29%29%0A%20%20%20%20%20%20%20%20print%28str%28p%29%5B0%5D%20%2B%20%22%20%20%20%20%22%20%2B%20str%28q%29%5B0%5D%20%2B%20%22%20%20%20%20%22%20%2B%20str%28material_implication%28p,%20q%29%29%5B0%5D%29&cumulative=false&heapPrimitives=false&mode=edit&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false
...
* $x + 1 = y$
//* The set $R = \{ x ∣  x \not\in x \}$ is an element of itself.
...
A goal of propositional logic is to have a method for creating valid arguments that are sequences of propositions, where the correctness and validity of the argument is based _solely_ on the propositions' *truth values* (True and False) 
// MKD Feb 23 2026 replace comma by whilte
//, 
while 
ignoring the actual content of the propositions. 
// MKD Feb 13 2025 commented out some text 
//Choosing to ignore the actual content of the propositions may seem strange at first, but c

// MKD Feb 23 2026
Compare this to doing algebra: 
//We can write $2 (x + 3 y) = 2x + 6y$ because we know that it is correct and valid to distribute multiplication over addition - we can do the algebra and ignore the actual real-world meaning of the "numerical content" that $x$ and $y$ stand for. 
You can write $2 (x + 3 y) = 2x + 6y$ because it is a correct and valid step to distribute multiplication over addition. You can do the algebra and ignore 
// MKD Feb 13 2025 commented out some text 
//the actual meaning/content (that is, 
the specific numerical values 
//) 
(the “numerical content”) that $x$ and $y$ stand for. 

//“”

In propositional logic, it is traditional to use *propositional variables* such as _p,_ _q,_ and _r_ to 
//represent 
stand for 
the possible assignments of truth values to propositions; 
//often, we refer to the variables themselves as the propositions. Again, compare this to the algebraic example where we can treat $x$ and $y$ as numbers even though they are actually variables that stand for numbers.
often, the propositional variables themselves are referred to as the propositions. Again, compare this to the algebraic example where you can treat $x$ and $y$ as numbers even though they are actually variables that stand for numbers.
//MKD moved this section of original book to appear earlier

//MKD mashed up some of the original text 9/22/2024
//MKD more mashing and rewording Feb 13 2025
//*Logical Translations*
What is the advantage of using symbols? 
A link:https://en.wikipedia.org/wiki/History_of_logic[long time ago,] philosophers discovered that is easier to follow lines of reasoning by putting our thoughts into symbols. This was an important step in the eventual development of modern technological society and, in particular,  electronic computers. Before a computer can do its work, humans need to put our thoughts into them; however, a spoken language like English can be too difficult to use because many different phrases can represent the same logical statements. 
//; translating statements from English sentences to symbols and back is a skill that needs lots of practice.

****
.Example {counter:logicexample} - Generalizing and abstracting an argument
//MKD inserted this example 2/13/24
//MKD Feb 13 2025 changed "we" to "you" throughout the example
...
This argument is 
//correct and 
valid: If you ASSUME that the first two propositions are True, then you can correctly conclude that the third proposition (which follows the introductory phrase "Therefore,") must be True as well.

Notice that you could change the name "Sarah" to "Daniel" without affecting the 
//correctness and 
validity of the argument. 

You can generalize the argument by changing "Sarah" to "the student." In fact, you can generalize much more by noticing that the _form_ of the argument matches the following argument.
...
// MKD edit 9/22/2024
You can build *compound propositions* (also called “propositional functions”) from simpler propositions. For example, in the preceding example, the compound propostion “if _p_ then _q_” was introduced as a new proposition built from the propositional variables _p_ and _q_. In the next section, you will learn how to represent compound propositions using symbols.


// MKD new section re-added here on 9/22/2024
...
// MKD new section added then omitted 9/22/2024
//== Compound Propostions and Truth Tables 
//== Logical Operations and Truth Tables 
...
A *truth table* can be used to display the truth values of a compound proposition that is built from propositional variables and logical operators. 
A truth table 
//typically 
is created with rows representing all possible *interpretations* of the propositional variables, that is, all possible assignments of truth values to the propositional variables. Each column of a truth table displays the truth values for either one of the propositional variables or a compound proposition built up from propositional variables and/or simpler compound propositions. As an analogy, think of how a table can be used to display the numerical input and output values of a function 
// MKD Feb 23 2026 
or formula that is 
...
[small]#Other textbooks and sources may use $\overline{p}$ or $\sim \! p$ to represent $\neg p.$# 
...
// MKD Feb 23 2026 
[small]#NOTE: For a proposition $p,$ exactly one of $p$ and $\neg p$ is True and exactly one is False. Two propositions can be _contrary_ (that is, the two propositions cannot both be True) without being negations of each other: 
As an example, only one of the propositions “Today is Friday” and “Today is Saturday” could be True, but both could be False! The proposition “Today is Friday” is not the negation of the proposition of “Today is Saturday.”#


//[small]#For a proposition $p,$ exactly one of $p$ and $\neg p$ is True and exactly one is False. Two propositions can be _contrary_ (that is, they could not both be True) without being negations of each other: As an example, both of the propositions “Today is Friday.” and “Today is Saturday.” could be False, so "Today is Saturday." is not the negation of "Today is Friday."#

////
From the truth table it can be seen that 
exactly one of the two propositions $p$ and $\neg p$ is True and 
exactly one is False.
Notice that, in each row of the truth table, exactly one of the two propositions $p$ and $\neg p$ is True and exactly one is False.
////

Notice that the two propositions $p$ and $\neg (\neg p)$ must always have the _same_ truth value (You can see this by inserting an additional column for $\neg (\neg p)$ in the truth table shown earlier in this subsection.)
...
Let $p$ and $q$ be propositions.  The *disjunction* of $p$ and $q$, denoted in mathematics by $p \lor q$ and read as “$p$ or $q$”, is True when _at least one_ of $p$ and $q$ are True and is False otherwise.
...
//MKD moved XOR to a subsection "Other Operations" and planning to include NAND and NOR there
...
Let $p$ and $q$ be propositions. The *conditional statement* $p \rightarrow q$, read as “if _p_ then _q_”, “_p_ implies _q_”, or, more formally, “the conditional with hypothesis _p_ and conclusion _q_”, is the proposition that is _False_ when _p_ is True and _q_ is False, and True otherwise. The conditional statement $p \rightarrow q$ is also called “the implication $p \rightarrow q$”. + 
//Other books and sources may use $p \Rightarrow q$ or $p \implies q$ for the conditional statement. 
[small]#The conditional $p \rightarrow q$ can also be denoted by $p \Rightarrow q$ or $p \implies q.$ In addition, there are many other ways to express the conditional $p \rightarrow q$ in English, two of which are “_p_ only if _q_” and “_q_ if _p_”.#
...
// MKD Sept 28 2025 inserted link to following truth table
[[conditional_converse_inverse_contrapositive]]
...
Notice that in this example, the conditional must be True, based on properties of factors of integers, but its converse could be False:  Consider the case where the number of students in class is equal to 26, so $p$ is False and $q$ is True, and $p\rightarrow q$ is True but $q\rightarrow p$ is False. This also shows, again, that the conditional need not represent a “cause-and-effect” relationship, since 
a number being NOT 
//being 
“divisible by 4” does not let you conclude anything about 
that number 
...
[small]#Note that $p \leftrightarrow q$ can also be denoted by $p \Leftrightarrow q$ or $p \iff q.$#
...
As a biconditional the example would say “You get a 100 on the final exam if and only if you earn an A in the class.” This becomes a two-way contract: You earn an A in the class if you get a 100 on the final, 
//_but_ 
_and_ you do not earn an A in the class if you do not get a 100 on the final. 
...
////
//MKD commented this out 9/22/2024
[NOTE]
====
...
//to build circuits in electronic components. 
//Each one of NAND and NOR is *functionally complete:* Any compound proposition is logically equivalent to a compound proposition involving only that one operator (See the Challenge Exercises at the end of this chapter.)

===== NAND 

//"I have *not both* a pencil and a pen."
// MKD Feb 5 2025 needed to replace the chimp example - it may be incorrect!
//		see https://en.wikipedia.org/wiki/Ape#Name_and_terminology
//"A chimpanzee is *not both* an ape and a monkey."
//	https://sciencenotes.org/is-tomato-a-fruit-difference-between-fruits-and-vegetables/
//"A cherry is *not both* a fruit and a vegetable."
“An link:https://en.wikipedia.org/wiki/Onion[onion] is *not both* a fruit and a vegetable.”

//Let $p$ and $q$ be propositions. The NAND of $p$ and $q$ is commonly denoted by $p \mid q$ and is False when both $p$ and $q$ are True and True otherwise. That is, the NAND is the negation of $p \land q.$ + 
//[small]#In this context, the symbol $\mid$ is called the “Sheffer stroke.” Other notations for NAND include $p{\overline {\land }}q$ and $p \uparrow q.$#
// MKD changed notation 9/22/2024 to avoid clashing with bitwise OR notation used in section from original book
...
// and," this is $\neg (p \land q).$
//The term "NAND" can be thought of "not and." 
...
//Notice that the two propositions $p \mid q$ and $q \mid p$ always have the same truth value.
...
// MKD Feb 5 2025 made relatively minor edits

//"This pen is *neither* black *nor* blue."
“This pen's ink is *neither* red *nor* blue.”

//Let $p$ and $q$ be propositions. The NOR of $p$ and $q$ is commonly denoted by $p \downarrow q$ and is True when both $p$ and $q$ are False and False otherwise. The NOR is also referred to as “joint denial” since it is True exactly when neither $p$ nor $q$ is True. That is, the NOR is the negation of $p \lor q$ - think of NOR as “Not OR.“” + 
//[small]#In this context, the symbol $\downarrow$ is called the "Peirce arrow." Another common notation for NOR is $p{\overline {\lor }}q.$#
...
The definition of wff allows you 
or, even better, a computer 
to analyze _any_ string of symbols to determine whether the string of symbols is a wff. For example, $(p \land q \lor r)$ isn't a wff but both $(p \land (q \lor r))$ and $((p \land q) \lor r)$ are wffs. You could write code to implement an algorithm to validate a string as a wff. 
...
It can be shown that every compound proposition can be represented by at least one wff. 
...
//FROM https://www.cs.rochester.edu/u/nelson/courses/csc_173/proplogic/expressions.html Precedence is (from highest to lowest): NOT, NAND, NOR, AND, OR, ->, ==.
...
// MKD note Mar 4 2026 
// Is this Jan Łukasiewicz's tautology?
...
is a tautology (that is, the compound proposition is always True.)
****

.The Law of Contradiction
****
Given any proposition $p$, the compound proposition \[p  \land \neg p\] 
is a contradiction (that is, the compound proposition is always False.)
**** 

// MKD Oct 19 2025 inserted anchor 
[[de_morgans_laws]]
=== De Morgan's Laws
...
Here is a link:https://www.mathematik.uni-marburg.de/~thormae/lectures/ti1/code/normalform/index.html[website] that allows you to build the DNF and CNF for a given propositional function.


//MKD Moved "Tautologies, Contradictions and Contingencies" from here to before De Morgan's laws


// MKD functional completeness begins
...
[click.Proof]
--
An informal justification can use the method shown in the previous examples for finding a DNF or CNF. 

A formal proof requires the link:./induction.html[mathematical induction proof technique] and the recursive defintion of well-formed formulae given earlier in this chapter.
--
...
****
// MKD functional completeness ends
...
//Instead of a single proposition, we have a function that takes names as inputs and gives proposition as out[ut]  to replace the individual with a variable it is often to tre
...
// MKD Feb 23 2026 replaced first bullet with ID number one
//*  For all integers $x$, $x^2\geq 0$.
*  Every student in the class was assigned an ID number.
*  Some student in the class has a birthday in July.

//Each of these statements considers a proposition over an entire population or set, called the *domain* or the *universe,* and  
// MKD Feb 23 2026 
Each of these statements is a proposition formed by considering the possible interpretations of variables of a predicate over a population or set, called the *domain* or the *universe.*  
// MKD Feb 13 2025 and Feb 23 2026
//quantifies how many elements (or people) 
One proposition describes whether _all_ the elements 
in the domain satisfy a first predicate, and  
and the other proposition describes whether _at least one_ element 
in the domain satisfies a second predicate. + 
[small]#The domain is also called the *domain of discourse* or *universe of discourse.*#
//To represent this idea, we use 


There are two commonly-used 
symbols for 
quantifiers, the *universal quantifier* and the *existential quantifier* symbols. + 
...
--
--
Let $P(x)$ be the statement $x + 1 \gt x$.  Is this true for _all_ integers x in the set $\{ -2, -1, 0, 1, 2 \}$?

include::python_tutor.adoc[]

NOTE: We cannot check for _all_ integers using code, so instead used the example domain $\{ -2, -1, 0, 1, 2 \}$.
****
...
--
--
Let $P(x)$ be the statement $x + x \gt x$.  Is this true for all integers x in $\{ -2, -1, 0, 1, 2 \}$?
...
Let $P(x)$ be the predicate "student $x$ was assigned an ID number".  
You can write the bulleted statement as $\forall x P(x)$, where the domain is the set of all students in the class. 
The quantified statement will be True as long as _every_ student in the class was assigned an ID number. It will be False, otherwise.

* Some student in the class has a birthday in July.

Let $Q(s)$ be the predicate "student $s$ has a birthday in July". 
You can write the bulleted statement as $\exists s Q(s)$, where the domain is the set of all students in the class. 
The quantified statement will be True as long as _at least one_ student in the class has a birthday in July. It will be False, otherwise.


=== Negation of Quantifiers
It is important to consider 
how to find 
the negation of a quantified expression. 
...
https://ggc-discrete-math.github.io/index.html[“Discrete Math”] book with minor edits to include base-two notation and a link to the NANDgame website.
...
//In binary $0+0=0$, and $1+0=0+1=1$, but, as in decimal addition, in binary $1+1=2$, which in binary will be a sum of $0$ and a carry of $1$ to the next significant column on the left.  
...
//MKD note added Feb 5 2025
[small]#Note: The bits are enclosed in parentheses which are followed by the subscript $_2$ to emphasize that binary notation, not decimal notation, is being used. This notation is covered in much more detail in the link:./number_bases.html[Number Bases] chapter.#

Thinking then of adding a specific column of two binary digits, say $A$ and
$B$, involves as input the 
//digits 
bits $A, B$ and the carry in 
from the previous column say $C_{in}$. 
The output
...
////
You can write logically equivalent expressions for $S$ and $C_{out}$ by first writing expressions in Disjucntive Normal Form and then simplifying those expressions using some of the other logical equivalences listed earlier in the chapter: 
\[S \equiv \left(\sim A\land \sim B\land C_{in}\right)\lor \left(\sim A\land B\land
\sim C_{in}\right)\lor \left(A\land \sim B\land \sim C_{in}\right)\lor \left(A\land B\land C_{in}\right) \] 
\[ C_{out} \equiv (A\land B)\lor \left(B\land C_{in}\right)\lor \left(A\land C_{in}\right)\] 
...
//[env.NANDgame]
[env.How.about.a.game.of.NAND?]
--
Here is a link:https://nandgame.com/[link] to a website that lets you build a computer, starting from the most basic level of the NAND component.
--
...
//. Show that each of the propositions $\neg p$, $p \land q,$ and $p \lor q$ is logically equivalent to a compound proposition that uses only the variables $p,$ $q,$ and the NAND operator $\uparrow.$ Conclude that _every_ compound proposition can be written 
