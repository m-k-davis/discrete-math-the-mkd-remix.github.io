...
_This chapter was last updated on September 23, 2024._
...
Let $p$ and $q$ be propositions. In this textbook, the NAND of $p$ and $q$ is denoted by $p \uparrow q$ and is False when both $p$ and $q$ are True and is True otherwise. That is, the NAND is the negation of $p \land q.$ 
...
//Let $p$ and $q$ be propositions. The NOR of $p$ and $q$ is commonly denoted by $p \downarrow q$ and is True when both of $p$ and $q$ are False and False otherwise. The NOR is also referred to as "joint denial" since it is True exactly when neither $p$ nor $q$ is True. That is, the NOR is the negation of $p \lor q.$ + 
...
[click.Proof]
--
An informal justification can use the method show in the previous examples for finding a DNF or CNF. 
...
https://ggc-discrete-math.github.io/index.html[“Discrete Math”] book with minor edits to include base-two notation.
...
.. Use the theorem that every compound proposition is equivalent to a compound proposition in disjunctive normal form. 
