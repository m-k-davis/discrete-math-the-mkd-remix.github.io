...
_This chapter was last updated on February 16, 2025_ + 
_Inserted example of "Proof By Exhaustion"_
...
If there are two cases, proof by exhaustion corresponds to using the "proof by cases" rule of inference discussed above in the section "Other Common Rules Of Inference." The tautology can be rewritten in the simpler form $((p \rightarrow q) \land (\neg p \rightarrow r)) \rightarrow r$ because $p \lor \neg p$ must always be True.
