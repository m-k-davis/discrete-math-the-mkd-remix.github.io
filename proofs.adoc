...
//MKD: Need to add info on Rules of Inference and/or chapter on Proof

== Rules Of Inference

//MKD need to define an argument somewhere

In order to make a valid argument, we must allow ourselves to proceed from true statements to other true statements without introducing false statements using faulty reasoning. 

To this end, we use *rules of inference*, which are ways to draw a conclusion from from one or more premises that are already assumed to be true (either as axioms or as previously-derived conclusions.)
...
//MKD get list of rules of inference from the te TWTask "Logic" and have students verify using truth tables.
...
Each rule is an established tautology of p1 ∧ p2 ∧ … ∧ pn → q
These rules of inference can be used in any mathematical argument and do not require any proof.
// MKD end slide

 

// MKD all content below is old and should be replaced/eliminated.
// note that the counter for examples has already been changed to match this chapter

== Propositional Logic

A *proposition* is a statement that declares a fact that is either True or False.

****
.Examples {counter:proofexample} - Propositions
...
****
.Example {counter:proofexample} - Negation in Python
--
--
...
****
.Example {counter:proofexample} - Conjunction in Python
--
--
...
****
.Example {counter:proofexample} - Disjunction in Python
--
--
...
****
.Example {counter:proofexample} - Exclusive Disjunction in Python
--
--
...
****
.Example {counter:proofexample} - Conditional, Converse, Contrapositive and Inverse.
...
****
.Example {counter:proofexample} - Bi-Implication in Python
--
--
...
****
.Example {counter:proofexample} - Tautology and Contradiction
...
****
.Example {counter:proofexample} - Predicates
...
:pt_code_height: 300
:pt_rawinput: false
:pt_code: def%20P%28x%29%3A%0A%20%20%20%20return%20str%28x%29%20%2B%20%22%20%E2%89%A4%203%22%0A%0Adef%20truth_value_of_P%28x%29%3A%0A%20%20%20%20return%20x%20%3C%3D%203%0A%0Aprint%28%20P%285%29,%22is%22,truth_value_of_P%285%29%29%0Aprint%28%20P%282%29,%22is%22,truth_value_of_P%282%29%29
****
.Example {counter:proofexample}
--
--
Let $P(x)$ be the predicate $x \leq 3$.  What are the propositions $P(5)$ and $P(2)$?  What are the truth values of $P(5)$ and $P(2)$?
...
What are the propositions $P(1,000)$ and $P(1,000,000)$ ?
...
****
.Example {counter:proofexample} - Negation of quantified statements
...
****
.Example {counter:proofexample} - Bitwise Operations
...
****
.Example {counter:proofexample} - Output of a logic circuit in terms Input
...
****
.Example {counter:proofexample} - Design a Logic Circuit
