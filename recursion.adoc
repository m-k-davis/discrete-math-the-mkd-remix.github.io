...
*Sequences* are functions with domain the natural numbers, or a subset of the natural numbers. That is, sequences are lists of objects indexed by some or all of the natural numbers. The indexed objects are called the *terms* of the sequence and may be any kind of object - numbers, sets, functions, strings, steps of a proof, steps of an algorithm, etc.
...
Notice that this sequence can be defined recursively using the recurrence relation \[a_0=1 \text{ and } a_n=a_{n-1}+3 \text{ for } n \in \mathbb{N}_{>0}.\] So
...
Notice that this sequence can be defined recursively using the recurrence relation \[b_0=2 \text{ and } b_n=3\cdot b_{n-1} \text{ for } n \in \mathbb{N}_{>0}.\] So
...
link:https://en.wikipedia.org/wiki/Fibonacci_sequence#History[Acharya Pingala,] link:https://www.sciencedirect.com/science/article/pii/0315086085900217[Acharya Virahanka,] 
and 
link:https://www.tandfonline.com/doi/abs/10.1080/10724117.2008.11974752[Acharya Hemachandra] 
...
[small]#Note: The definition used in this textbook matches the ones used in several textbooks, but be warned that other may use definitions that are slightly different (e.g., some sources state the initial values as  $f_{0}=f_{1}=1.$#
...
called a *closed form,* for the terms of the sequence.
