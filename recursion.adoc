...
*Sequences* are functions with domain the natural numbers, or a subset of the natural numbers. That is, sequences are lists of objects indexed by some or all of the  natural numbers. The indexed objects are called the *terms* of the sequence and may be numbers, but could be any other kind of object like a set, a function, strings, etc.
...
****
.Example {counter:indrecexample} - The Fibonacci sequence
--
--
...
The preceding image shows the basis step and represents, in part, the first and second iterations of the recursion step; note that infinitely-many rooted trees are constructed at each iteration of the recursion step, so we cannot show all the rooted trees produced at any step other than the basis step. Also, we would need to complete infinitely-many steps to construct _all possible_ rooted trees, but any one particular rooted tree you want to construct will be produced after finitely many iterations of the recursion step. 
