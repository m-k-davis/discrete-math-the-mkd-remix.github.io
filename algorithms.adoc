...
This long division algorithm uses powers of two (instead of ten) but otherwise is like the algorithm you may have learned in school. The worst-case scenario corresponds to the divisor $b=1,$ which requires you to shift as many times as there are _binary_ digits in $a.$ Notice that $a$ is bounded above by 2 raised to the number of binary digits; this means that 
...
// MKD Apr 01 2025
You can use link:./induction.html[mathematical induction] to prove that $1+2+3+\cdots+n-1= \frac{(n-1)\cdot\ n}{2}=\frac{n^2}{2}-\frac{n}{2}$, which means that the bubble sort algorithm is $O(n^2)$.
