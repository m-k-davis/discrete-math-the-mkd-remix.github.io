...
Notice that the "division by repeated subtraction" has a worst-case scenario when the divisor _b_ equals 1. If _n_ is the number _a,_ then it should be clear that "division by repeated subtraction" is $O(n)$.
...
This long division algorithm uses powers of two (instead of ten,) but otherwise is like the algorithm you may have learned in school. The worst-case scenario is when the divisor _b_ equals 1, which requires use to shift as many times as there are _binary_ digits in $a,$ and $a$ is bounded above by 2 to the power the number of binary digits. This means that if _n_ is the number _a,_ then this long division algorithm is $O(\log (n)).$
...
Note that Python provides built-in operators to find the quotient and remainder for variables `a` and `b` of type `int`: `a//b` is the quotient and `a%b` is the remainder. There is also a less effiecient operator `divmod(a,b)` that returns an ordered pair (of data type `tuple`) containing the quotient and the remainder.
