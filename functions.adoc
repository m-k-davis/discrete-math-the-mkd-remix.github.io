...
_This chapter was last updated on September 12, 2024._
...
a function $f$ from set _D_ to set _C_ is a rule that assigns to each input element in _D_ exactly one output element from _C._ The set _D_ is called the domain of the function, and the set _C_ is called the codomain of the function. This informal definition was introduced in the link:introduction_discrete_math.html#informal_function_definition[ Introducing Discrete Mathematics] chapter.
...
Notice that the informal definition states that _every_ element in the domain _D_ is assigned an output value in the codomain _C._ #The informal definition does not specify whether _every_ element in the codomain _C_ is an output for some input in the domain _D._# 
...
In computing, integer data types are used to represent integers such as loop counters or indices into arrays, while floating-point data types are used to represent real numbers used in scientific or financial calculations. 
...
//Even though most 
In general, numbers that can be stored using an integer type can also be stored using a floating-point type, 
but 
...
//Into the weeds: 
On most hardware, both Python's `float` data type and Java's `double` data type are implementations of 
link:https://ieeexplore.ieee.org/document/8766229[IEEE 754]  double precision 64-bit floating-point numbers. The link:https://docs.python.org/3/library/math.html#math.floor[floor() function in Python] takes a `float` as input and returns an `int` and the link:https://docs.oracle.com/javase/8/docs/api/java/lang/Math.html#floor-double-[floor() function in Java] takes a `double` as input and returns a `double.` 
This means that the floor() functions in Python and Java have the _same_ domain and use, essentially, the _same_ rule to compute output values, but have _different_ codomains.# 
...
Now notice that in the Java code below, that the return value from the floor() function is a _double_ which we cannot use as an index into array L without error. We must first use another function to cast the _double_ to type _int_ to get the correct data type for an array index.
...
[small]#This definition of a function as an ordered triple is based on the  Bourbaki definition in the 1970 book 
...
#The reason why the ordered triple is used in the definition is that we need to be able to distinguish two functions that have the same graph 
...
can all be described by the words "the output is the square of the input" 
and  
//because they are 
have 
//nearly 
